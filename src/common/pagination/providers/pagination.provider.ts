...
export class PaginationProvider {
	...
	async paginateQuery<T extends ObjectLiteral>(
			pagingQuery: GetServiceFilterQueryDto & { userId?: number },
			repository: Repository<T>
	): Promise<Pagination<T>> {
		const { page, limit, search, category, minPrice, maxPrice, userId } = pagingQuery;
		...
		if (pagingQuery.userId) {
			const hasUserRelation = repository.metadata.relations.some(
					(r) => r.propertyName === "user"
			);

			if (hasUserRelation) {
				queryBuilder.andWhere(`${alias}.user.id = :userId`, { userId });
			} else if (repository.metadata.columns.some((c) => c.propertyName === "userId")) {
				queryBuilder.andWhere(`${alias}.userId = :userId`, { userId });
			}
		}
